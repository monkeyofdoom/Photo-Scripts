...
import os, time, sys, string, subprocess
# from stat import * # ST_SIZE etc

extlist = [".JPG",".CR2",".DNG",".MOV",".AVI",".3GP"]
movietypes = [".MOV",".AVI"]     
...
if os.path.exists(path):
	...
	for infile in listing:
		...
		if ext[1].upper() in extlist:
			...
			fo = subprocess.Popen(["exiftool", "-S", "-s", "-SubSecCreateDate", path+infile], shell=True, stdout=subprocess.PIPE)

			createdate = fo.communicate()[0]
			...
			if createdate == "":
				fo = subprocess.Popen(["exiftool", "-S", "-s", "-CreateDate", path+infile], shell=True, stdout=subprocess.PIPE)
				createdate = fo.communicate()[0]
				createdate = createdate.strip()
				createdate = string.replace(createdate,":","-")
				createdate = string.replace(createdate," ","_")
			...
			if createdate != "":
				...
				if renamefrom != renameto:
					if not os.path.isfile(renameto):
						...
						print "skipped file "+ infile +" <<<< FILE EXISTS"
				else:
					print "skipped file "+ infile +" - already named correctly"
